...
  // Fetch detail berita (tidak ada perubahan di sini)
  useEffect(() => {
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <header className="mb-8">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mb-6 border-b pb-4">
                    ...
                    {/* MODIFIKASI 1: Membuat kategori bisa diklik */}
                            ...
                            <div className="flex flex-wrap gap-2">
                                {news.categories.map(cat => (
                                    <Link 
                                        ...
                                        className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 hover:bg-blue-200 hover:text-blue-900 transition-colors"
                                    >
                                        {cat.name}
                                    </Link>
                                ))}
            ...
            <article className="space-y-6">
                {allContent.length === 0 ? (
                    <div className="text-center py-8 text-gray-500"><p>Tidak ada konten untuk ditampilkan.</p></div>
                ) : (
                    allContent.map((item, index) => {
                        if ('type' in item && item.type === 'image') {
                            ...
                            return (
                                <figure key={`image-${imageItem.id}-${index}`} className="mb-8">
                                    ...
                                    />
                                    {imageItem.caption && (
                                        <figcaption className="text-center text-gray-600 text-sm mt-3 italic">{imageItem.caption}</figcaption>
                                    )}
                                </figure>
                            );
                        }
                                    ...
                                    <div className="flex items-start gap-4">
                                        <div className="flex-shrink-0">
                                            <div className="w-12 h-12 bg-red-100 rounded-lg flex items-center justify-center">
                                                <svg className="w-6 h-6 text-red-600" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" /></svg>
                                            </div>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-lg font-semibold text-gray-800 mb-1">{pdfItem.title || pdfItem.original_name || 'Dokumen PDF'}</h4>
                                            {pdfItem.description && (<p className="text-gray-600 mb-2">{pdfItem.description}</p>)}
                                            <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                                                {pdfItem.file_size && (<span>Ukuran: {formatFileSize(pdfItem.file_size)}</span>)}
                                                <span>Format: PDF</span>
                                            </div>
                                            {/* MODIFIKASI 2: Menghilangkan tombol 'Lihat PDF' */}
                        ...
                        if ('content' in item) {
                            ...
                            if (contentItem.type === 'heading') {
                                return (<h2 key={`content-${contentItem.id}-${index}`} className="text-2xl font-bold text-gray-800 mb-4 mt-8">{contentItem.content}</h2>);
                            }
                            if (contentItem.type === 'paragraph') {
                                return (<div key={`content-${contentItem.id}-${index}`}><FormattedContent content={contentItem.content} /></div>);
                            }
                        }
                        return null;
                    })
